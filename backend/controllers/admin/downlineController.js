...
export const getMyReportByEvents = async (req, res) => {
  ...
  try {
    ...
    // Convert to strings for betHistoryModel (userId stored as String)
    const downlineIdStrings = downlineIds.map((id) => id.toString());
    ...
    // Use betHistoryModel as single source of truth for P/L reporting
    const betQuery = {
      userId: { $in: downlineIdStrings },
      status: { $in: [1, 2] },
    };
    ...
    if (betHistoryMode || betHistoryIndividualMode) {
      ...
      // Use betHistoryModel as single source of truth for P/L reporting
      // betHistoryModel stores each individual bet with original values,
      // unlike betModel which may have zeroed-out offset positions
      bets = await betHistoryModel
        .find(betQuery)
        .sort({ createdAt: -1 })
        .lean();
...
export const getMyReportByDownline = async (req, res) => {
  ...
  try {
    ...
    const userProfitAggregation = await betHistoryModel.aggregate([
      { $match: betQuery },
      {
        $group: {
          _id: '$userId',
...
export const getBetHistory = async (req, res) => {
  ...
  try {
    ...
    if (selectedVoid === 'settel') {
      ...
      query.status = 3; // Voided bets (status = 3)
...
export const getUsersWinLoss = async (req, res) => {
  const { id } = req;
  const { startDate, endDate } = req.query;
  ...
  try {
    ...
    const downlineResult = await SubAdmin.aggregate([
      { $match: { _id: admin._id } },
      {
        $graphLookup: {
          from: 'subadmins',
    ...
    const allDownline = downlineResult[0]?.downline || [];
    const users = allDownline.filter((u) => u.role === 'user');
    const userIds = users.map((u) => u._id.toString());

    if (userIds.length === 0) {
      return res.status(200).json({ success: true, data: [] });
    }
    const aggregation = await betHistoryModel.aggregate([
      {
        $match: {
          userId: { $in: userIds },
          status: { $in: [1, 2] },
          date: getDateRangeUTC(startDate, endDate),
        },
      },
      {
        $group: {
          ...
          casinoPL: {
            $sum: {
              $cond: [{ $eq: ['$betType', 'casino'] }, '$profitLossChange', 0],
            },
          },
          sportsPL: {
            $sum: {
              $cond: [{ $ne: ['$betType', 'casino'] }, '$profitLossChange', 0],
            },
          },
        },
      },
    ...
    aggregation.forEach((entry) => {
      plMap.set(entry._id, {
        casinoPL: entry.casinoPL,
        sportsPL: entry.sportsPL,
      });
    });

    const data = users.map((user) => {
      const pl = plMap.get(user._id.toString()) || {
        casinoPL: 0,
        sportsPL: 0,
      };
      return {
        userId: user._id,
        userName: user.userName,
        casinoPL: parseFloat(pl.casinoPL.toFixed(2)),
        sportsPL: parseFloat(pl.sportsPL.toFixed(2)),
        totalPL: parseFloat((pl.casinoPL + pl.sportsPL).toFixed(2)),
      };
    });

    return res.status(200).json({ success: true, data });
  } catch (error) {
    console.error('getUsersWinLoss error:', error);
