...
async function run() {
  ...
  const admins = await SubAdmin.find({ role: { $nin: ['supperadmin', 'superadmin', 'user'] } });
  if (!admins.length) {
  ...
  console.log("Admin Summary for", admin.userName, ":", JSON.stringify(summary.accountSummary, null, 2));
