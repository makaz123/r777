...
const PLDisplayNoLay = memo(function PLDisplayNoLay({
  ...
  return (
    <div className='flex gap-2 text-[11px]'>
      ...
      {isOppositeTeam &&
        sectionBet?.totalPrice < 0 &&
        (() => {
          const guaranteedBet = sectionBet;
          return (
            <div>
              P:
              <span className='font-bold text-green-600'>
...
const Lucky7Section = [
  ...
  { key: 'luckyCard', title: 'LUCKY CARD', type: 'card' },
];

const getCardValue = (nat, gameid) => {
  if (!nat) return '';

  // Lucky7 format: "Card 7", "Card Q"
  if (gameid === 'lucky7eu') {
    const match = nat.match(/Card (.+)/i);
    return match ? match[1] : nat;
  }

  // DragonTiger format: "Dragon Card A", "Tiger Card 10"
...
const TableRow = memo(function TableRow({
  ...
  const isOppositeTeam = !hasPendingBet && sectionBet !== null;
    ...
    <>
      ...
      >
        ...
        >
          ...
          <span className='text-[11px]'>{item.bs}</span>
        </div>
...
const BettingSection = memo(function BettingSection({
  ...
  const sectionBet = items.reduce((found, itm) => {
    if (found) return found;
    return getBetDetails(pendingBetAmounts, itm.nat, itm.sid);
  }, null);
  ...
  const getDisplayContent = (item) => {
    switch (type) {
      ...
      case 'card': {
        const cardValue = getCardValue(item.nat, gameid);
...
const DragonTigerRenderer = memo(function DragonTigerRenderer({
  ...
  const isLucky7 = gameid === 'lucky7eu';

  const sections = useMemo(() => {
    if (isLucky7) {
      return groupLucky7Data(bettingData?.sub);
    }
    return groupDragonTigerData(bettingData?.sub);
  }, [bettingData?.sub, isLucky7]);

  const sectionConfig = isLucky7 ? Lucky7Section : DRAGON_TIGER_SECTIONS;
