...
function OverUnder_25({
  ...
  const oddsData = matcUnder25List?.[0]?.section?.length
  ...
  const getBetDetails = (team) => {
    ...
    const otherTeamBet = pendingBetAmounts?.find(
      (item) => item.gameType === 'OVER_UNDER_25'
    );
  ...
  const handleOddsClick = (team, rate, type, sid, oname) => {
    if (onBetSelect && rate) {
      ...
      onBetSelect({
        ...
        maxAmount: matcUnder25List?.[0]?.max || matcUnder25List?.[0]?.maxb || 0,
        minAmount: matcUnder25List?.[0]?.min || 0,
      });
    }
  };

  // Get max value from API
  const maxValue = matcUnder25List?.[0]?.max || matcUnder25List?.[0]?.maxb || 0;
  return (
    <div>
        ...
        <div>
          <button
            ...
            className={`p-1 font-[400] text-white ${
              showCashoutOptions
                ? 'cursor-pointer bg-black px-2.5 py-0.5 rounded-[3px] text-[12px] font-[600] text-white'
                : 'cursor-not-allowed bg-[#959595] px-2.5 py-0.5 rounded-[3px] text-[12px] font-[600] text-white'
            }`}
          >
            Cashout
          </button>
      ...
      {oddsData.length > 0 ? (
        oddsData.map(({ team, odds, sid, gstatus }, teamIndex) => {
          ...
          const isOverUnderMarket =
            selectedBet?.gameType === 'OVER_UNDER_25' ||
            selectedBet?.marketName === 'OVER_UNDER_25';
              ...
              <div className='flex border-b border-b-[#c7c8ca] hover:bg-[#f7f7f7]'>
                ...
                <div className='ml-2 flex flex-1 flex-col justify-center truncate text-[13px] font-bold text-black lg:text-[14px]'>
                  ...
                  {(() => {
                    ...
                    // Check if selectedBet belongs to OverUnder_25 market
                ...
                {[0, 1, 2].map((i) => {
                  ...
                  return (
                    <div
                      key={`lay-${i}`}
                      className={`${layBg[i]} m-[1px] w-1/2 md:w-1/3 hidden min-h-[36px] max-w-[100%] flex-col items-center justify-center rounded-[3px] md:flex ${formattedOdds ? 'cursor-pointer transition-opacity hover:opacity-80' : ''}`}
                      onClick={() =>
                        formattedOdds &&
                        handleOddsClick(
                          team,
              ...
              {showInlineBetSlip && (
                <div className='border-b border-b-[#c7c8ca] bg-[#fafafa]'>
                  <PlaceBet
                    ...
                    gameId={gameid}
                    eventName={matchEventName}
                    marketName={selectedBet?.marketName || 'OVER_UNDER_25'}
